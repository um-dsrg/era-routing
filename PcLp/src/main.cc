  ...
  try
    {
      ...
      cmdLineParams.add_options ()
        ...
        ("output", po::value<std::vector<std::string>> (&outputPaths)->multitoken (),
         "List of paths where to save the result of each of the given optimisation "
         "problem")
        ("considerAckFlows", po::bool_switch (&considerAckFlags)->default_value(false),
         "When set, the Lp solution will also take into consideration the ACK flows")
        ("optimisationProblems",
         po::value<std::vector<std::string>> (&optimisationProblems)->multitoken (),
         "List of optimisation problems to run. Available names are: MaxFlowOnly | "
         "MinCost | FlowLimitedMinCost | MaxDelayDistribution. Note that all of the "
         "options will run the Maximum Flow before hand. If multiple optimisation "
         "problems are given the Maximum Flow solution will be kept constant for all "
         "the different runs. This is important when comparing solutions with each "
         "other. Each different run generates its own XML result file.");
      // clang-format on
      ...
      if (optimisationProblems.size () != outputPaths.size ())
        {
          throw std::runtime_error (
              "The number of optimisation problems and output paths needs to be "
              "identical");
        }

      LpSolver lpSolver (links, paths, considerAckFlags);
          ...
          if (optimisationProblem == "MaxFlowOnly")
          ...
          else
            {
              ...
              if (optimisationProblem == "MinCost")
                ...
                {
                  std::cout << "Solving the Maximum Flow Minimum Cost problem with each flow's "
                               "assigned data "
