  ...
  try
  {
    ...
    cmdLineParams.add_options()
                  ...
                  ("optimisationProblems",
                   ...
                   "List of optimisation problems to run. Available names are: MinCost | "
                   "FlowLimitedMinCost | MaxDelay. Note that all of the options will run the "
                   "Maximum Flow before hand. If multiple optimisation problems are given the "
                   "Maximum Flow solution will be kept constant for all the different runs. "
                   "This is important when comparing solutions with each other. Each different run "
                   "generates its own XML result file.")
                  ("output",
    ...
    for (auto index = 0; index < optimisationProblems.size(); ++index)
    {
      ...
      else if (optimisationProblem == "MaxDelay")
      {
        std::cout << "Solving the Maximum Flow Maximum Delay metric problem" << std::endl;
        optimalSolutionFound = lpSolver.MaxDelayMetric(maxNetworkFlow);
      }
      else
      {
